...
import { Armchair, Activity, Wrench, Archive, Building2, Clock, AlertTriangle } from 'lucide-react'
...
export default function UtilityDashboardPage() {
  ...
  const { data: defectiveCount } = useQuery({
    queryKey: ['utility-defective-count'],
    queryFn: async () => {
      const res = await assetService.getDashboardStats('utility')
