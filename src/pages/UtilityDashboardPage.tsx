...
import { Armchair, Activity, Wrench, Archive, Building2, Clock, Wind } from 'lucide-react'
...
export default function UtilityDashboardPage() {
  ...
  const { data: fanAcCount } = useQuery({
    queryKey: ['utility-fan-ac-count'],
    queryFn: async () => {
      const [fans, acs] = await Promise.all([
        assetService.getByDomain('utility', { category: 'Stand Fans' }, 1, 1),
        assetService.getByDomain('utility', { category: 'Air Conditioners' }, 1, 1),
      ])
      return (fans?.count || 0) + (acs?.count || 0)
    },
    staleTime: 0,
    refetchOnWindowFocus: true,
  })
  ...
  const statCards = [
    { title: 'Total Utility Assets', value: stats?.totalAssets       ?? 0, icon: Armchair,  color: '#10b981', gradient: 'linear-gradient(135deg,#10b981,#06b6d4)' },
    { title: 'Active',               value: stats?.activeAssets      ?? 0, icon: Activity,  color: '#3b82f6', gradient: 'linear-gradient(135deg,#3b82f6,#6366f1)' },
    ...
    { title: 'Fans & ACs',           value: fanAcCount               ?? 0, icon: Wind,      color: '#06b6d4', gradient: 'linear-gradient(135deg,#06b6d4,#3b82f6)' },
  ]
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
        ...
        <div className="p-5 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
          <div className="flex items-center gap-2">
            ...
            <h3 className="section-title">Recently Added Utility Assets</h3>
          </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {recentAssets.length === 0 ? (
                <tr><td colSpan={6} className="px-5 py-8 text-center text-sm" style={{ color: 'var(--text-muted)' }}>No utility assets yet</td></tr>
              ) : recentAssets.map((asset: any, i: number) => (
                <tr key={i} className="table-row-hover" style={{ borderTop: '1px solid var(--border-subtle)' }}
                  onClick={() => window.location.href = `/utility-assets/${asset.id}`}>
